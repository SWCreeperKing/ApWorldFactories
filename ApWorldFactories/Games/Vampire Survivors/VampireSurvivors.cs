...
public class VampireSurvivors : BuildData
{
        ...
        EnemyHurryMap = rawEnemyMap
                       .Where(t => t.Minute >= 15
                                   && !rawEnemyMapGrouping
                                      ...
                                      .Any(subT => subT.Item2 == t.Item2 && subT.Minute < 15)
                        ).GroupBy(t => t.Item1)
                       .ToDictionary(
                            g => g.Key,
    ...
    public override void Options(WorldFactory _, OptionsFactory options_fact)
    {
        options_fact.AddOption(
                         ...
                         """, new Choice(0, "stage_hunt", "director")
                     ).AddOption(
                         "Chest Checks Per Stage",
                         """
                         ...
                         2 = fully unlocked
                         """, new Choice(1, "disabled", "locked_behind_item", "unlocked")
                     ).AddOption("Lock Hyper Behind Item", "Lock the `Hyper` gamemode behind an item", new Toggle())
                    .AddOption(
                         "Lock Hurry Behind Item",
                         """
        ...
        options_fact
           .InjectCodeIntoOptionsClass(py =>
                py
                   .AddMethod(
                        new MethodFactory("flatten_locations")
                   ...
                   .AddMethod(
                        new MethodFactory("get_included_characters")
                   ...
                   .AddMethod(
                        new MethodFactory("get_included_stages")
                           .AddParams("self", "world")
                           .AddCode(
            ...
            options_fact
               .AddOption(
                    ...
                    $"""
                     ...
                     """, new OptionSet("\"All\"", $"{included.OptionVariableFormat()} + [\"All\", \"Random\"]")
                );
    ...
    public override void Locations(WorldFactory _, LocationFactory location_fact)
    {
        ...
        location_fact
           ...
           .AddLocations(
                "beat_with_character", CharacterData.GetNames().Select(s => (string[])[$"Beat with {s}", "Characters"])
            )
           .AddLocations(
                "open_chests",
                StageNameMap.Values.SelectMany(s
                    => Enumerable.Range(1, 10).Select(i => (string[])[$"Open Chest #{i} on {s}", s])
                )
            )
           .AddLocations("kill_enemies", EnemyMap.Keys.Select(s => (string[])[$"Kill {s}", "Enemies"]));
        ...
        void AllowClassifiers<T>(string name, T[] array, int type)
            where T : IClassifier, IGetName
        {
            location_fact.AddIndependentVariable(
                new StringArray(
                    name.OptionVariableFormat(),
    ...
    {
        item_fact.AddIndependentVariable(
                      new Variable(
                          "unlock_character_items",
                 ...
                 .AddCreateItems(method
                      => method
                        ...
                        .AddCode(
                             new ForLoopFactory("stage", "stages")
                                .AddCode(new IfFactory("stage == world.starting_stage").AddCode("continue"))
                                .AddCode("pool.append(world.create_item(f\"Stage Unlock: {stage}\"))")
                         )
                        .AddCode(
                             new ForLoopFactory("character", "characters")
                                .AddCode(new IfFactory("character == world.starting_character").AddCode("continue"))
                                .AddCode("pool.append(world.create_item(f\"Character Unlock: {character}\"))")
                         )
                        .AddCode(
                             new IfFactory("options.lock_hurry_behind_item").AddCode(
                                 CreateItem("Gamemode Unlock: Hurry")
                             )
                         )
                        .AddCode(
                             new IfFactory("options.lock_hyper_behind_item").AddCode(
                                 CreateItem("Gamemode Unlock: Hyper")
                             )
                         )
                        .AddCode(
                             new IfFactory("options.lock_arcanas_behind_item").AddCode(
                                 CreateItem("Gamemode Unlock: Arcanas")
                             )
                         )
                        .AddCode(new IfFactory("options.egg_inclusion").AddCode(CreateItem("Gamemode Unlock: Eggs")))
    ...
    public override void Rules(WorldFactory _, RuleFactory rule_fact)
    {
        rule_fact.AddLogicFunction("hasN", "has_amount", StateHas("item", "amount", false), "item", "amount")
                 ...
                 .AddCompoundLogicFunction(
                      "anyStage", "has_any_stage", "any[[f\"Stage Unlock: {stage}\" for stage in stages]]", "stages"
                  )
                 .AddCompoundLogicFunction(
                      "gamemode", "has_gamemode", "has[f\"Gamemode Unlock: {gamemode}\"]", "gamemode"
                  )
    ...
    public override void Init(WorldFactory worldFactory, WorldInitFactory init_fact)
    {
        init_fact
           ...
           .AddUseUniversalTrackerPassthrough(method => method.AddCode(UtGenEarly), false, false)
           ...
           .UseSetRules(method =>
                method.AddCode(
                    new IfFactory("self.options.goal_requirement == 0").AddCode(
                        CreateGoalCondition(
                            "hasN[\"Beat a Stage\", self.stage_goal_amount]", worldFactory.GetRuleFactory()
                        )
                    ).AddElseIf(
                ...
                world.AddVariable(new Variable("gen_puml", "False"))
                     .AddVariable(
                          new MappedVariable<string, string>(
                              ...
                              {
                                  ["\"unlocks\""]
                                      = "unlock_character_items + unlock_stage_items + unlock_gamemodes"
                              }
                          )
                      )
            )
    ...
    )
    {
        return new GraphBuilder(GameName)
              ...
              .ForEachOf(
                   StageNameMap,
                   (b, kv) => b.AddEventLocation(
                       kv.Value, getRule, $"Event: [{kv.Value} Beaten]", $"{kv.Value} Beaten", "Beat a Stage"
                   )
               )
              .GenString();
    }
