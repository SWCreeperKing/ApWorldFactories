...
public class VampireSurvivors : BuildData
{
    ...
    public override void Options(WorldFactory _, OptionsFactory options_fact)
    {
        ...
        void AddClassifierOptionsSet<T>(string included, int selector, T[] array, bool useAll)
            where T : IClassifier, IGetName
        ...
        void AddVariableOptionsSet<T>(string included, T[] array, bool useAll) where T : IGetName
        {
            options_fact
               .AddOption(
                    ...
                    new OptionSet(
                        [useAll ? "\"All\"" : ""],
                        [$"*{included.OptionVariableFormat()}", "\"All\"", "\"Random\""]
                    )
                );
                 ...
                 .AddCreateItems(method => method
                                          ...
                                          .AddCode(
                                               new ForLoopFactory("stage", "stages")
                                                  .AddCode(
                                                       new IfFactory("stage == world.starting_stage").AddCode(
                                                           "continue"
                                                       )
                                                   )
                                          ...
                                          .AddCode(
                                               new ForLoopFactory("character", "characters")
                                                  .AddCode(
                                                       new IfFactory("character == world.starting_character")
                                                          .AddCode("continue")
                                                   )
                                                  .AddCode(
                                          ...
                                          .AddCode(
                                               new IfFactory("options.egg_inclusion").AddCode(
                                       ...
                                       new IfFactory("options.enemysanity")
                                          .AddCode(
                                               new ForLoopFactory("enemy, raw_find_locs", "enemy_map.items()")
                                                  ...
                                                  .AddCode(
                                                       new IfFactory("not any(loc in stages for loc in raw_find_locs)")
                                                          .AddCode("continue")
                                                   )
                                                  .AddCode(
    ...
    public override void Init(WorldFactory worldFactory, WorldInitFactory init_fact)
    {
        init_fact
                ...
                method => method
                         ...
                         .AddCode(
                              CreatePushPrecollected(
                                  "f\"Character Unlock: {self.starting_character}\"", stringify: false
                              )
                          )
                         .AddCode(
                              CreatePushPrecollected(
                                  "Gamemode Unlock: Arcanas", "self.options.lock_arcanas_behind_item.value"
                              )
                          )
                         .AddCode(
                              CreatePushPrecollected(
           ...
           .UseSetRules(method =>
                method.AddCode(
                    new IfFactory("self.options.goal_requirement == 0")
                       .AddCode(
                            CreateGoalCondition(
                                "hasN[\"Beat a Stage\", self.stage_goal_amount]",
        ...
        return new GraphBuilder(GameName)
              ...
              .ForEachOf(
                   ...
                   (b, kv) => b.AddEventLocation(
                       kv.Value, getRule, $"Event: [{kv.Value} Beaten]", $"{kv.Value} Beaten",
                       "Beat a Stage"
                   )
               )
              .GenString();
