...
public class Atlyss : BuildData
{
    ...
    private List<string> KindsOfItems = [];
    ...
    public override void Items(WorldFactory _, ItemFactory item_fact)
    {
        ...
        GetMaxProgressive(
            "Fighter", ClassType.Fighter, progressiveItemMapMd, item_fact, progressiveItemMap,
            out var addFighter
        );
        progressiveItemMapMd.Add("\n---\n");
        GetMaxProgressive(
            "Mystic", ClassType.Mystic, progressiveItemMapMd, item_fact, progressiveItemMap,
            out var addMystic
        );
        progressiveItemMapMd.Add("\n---\n");
        GetMaxProgressive(
            "Bandit", ClassType.Bandit, progressiveItemMapMd, item_fact, progressiveItemMap,
            out var addBandit
        );
        ...
        item_fact
           ...
           .AddCreateItems(factory =>
                factory
                   ...
                   .AddCode(addBandit)
                   .ForEachOf(
                        KindsOfItems, item => factory.AddCode(CreateItemsFromMapCountGenCode($"item_counts_{item.ToLower()}"))
                    )
                   .AddCode(
                   ...
                   .AddCode(
                        CreateItemsFillRemainingWithItem(
                            "random.choices(filler_items, filler_weightings)[0]",
                            false
                        )
                    )
            );
    ...
    public override void Rules(WorldFactory _, RuleFactory rule_fact)
    {
        rule_fact
           ...
           .AddLogicRules(
                MerchantData.SelectMany(data => Enumerable.Range(1, 5).Select(i =>
    ...
    public override void Regions(WorldFactory worldFactory, RegionFactory region_fact)
    {
        region_fact.AddRegions("", LocationLevelData.Select(data => data.Area).ToArray())
                   ...
                   .AddEventLocationsFromList(
                        ...
                        "f\"Complete: {location[0]}\""
                    ).ForEachOf(
                        AchievementData
                           .Where(data => data is { Enabled: true, Class: not ClassType.Any })
                                  ...
                                  CreateGoalCondition(
                                      string.Join(
                                          ...
                                          EnemyListData.Where(data => data.IsBoss)
                                                       .Select(data => $"enemy[\"{data.Name}\"]")
                                      ), rule_fact
                                  )
                              )
                             .AddCase(
                                  "7", CreateGoalCondition(
                                      string.Join(
                                          " and ", QuestData
    ...
    private void GetMaxProgressive(string className, ClassType classType, List<string> progressiveItemMapMd,
        ItemFactory item_fact,
        ...
        return new GraphBuilder(GameName)
              ...
              .ForEachOf(
                   ...
                   (b, data) => b.AddEventLocation(
                       data.AreaAccepted, getRule, $"Quest Completion: {data.Quest}",
                       data.Quest,
                       $"Complete: {data.Quest}"
                   )
               ).GenString();
