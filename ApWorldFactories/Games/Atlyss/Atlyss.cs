...
public class Atlyss : BuildData
{
    ...
    public override void Options(WorldFactory _, OptionsFactory options_fact)
    {
        options_fact.AddOption("Goal",
                         ...
                         """,
                         new Choice(0, "slime_diva", "lord_zuulneruda", "colossus", "galius", "lord_kaluuz", "valdur",
                             "all_bosses", "all_quests", "level_32")).AddOption("Random Portals",
                         """
                    ...
                    .AddOption("Shop Sanity",
                         ...
                         """, new DefaultOnToggle())
                    .AddOption("Main Class", "What you chose to be as your main class",
                         new Choice(0, "fighter", "bandit", "mystic"))
                    .AddOption("Secondary Class", "What you chose to be as your secondary class",
                         new Choice(0, "fighter", "bandit", "mystic", "none"))
                    .InjectCodeIntoOptionsClass(classFact => classFact.AddMethod(new MethodFactory("is_class")
    ...
    public override void Locations(WorldFactory _, LocationFactory location_fact)
    {
        location_fact
           ...
           .AddLocations("levels", Enumerable.Range(1, 16).Select(i => (string[])[$"Reach Level {i * 2}", "Menu"]))
           .AddLocations("merchants",
                MerchantData.SelectMany(data => Enumerable.Range(1, 5)
                                                          .Select(i => (string[])
                                                           [
                                                               $"Buy Item #{i} from {data.Name}", data.Area
                                                           ]))).AddLocations("professions",
                ProfessionsData.Select(data => data.Profession).Distinct().SelectMany(s
                    => Enumerable.Range(1, 10).Select(i => (string[])[$"{s} Lv. {i}", "Menu"]))).AddLocations(
                "achievements",
           ...
           .AddIndependentVariable(GetGrindData("fishing",
                ProfessionsData.Where(data => data.Profession is "Fishing")
                               .SelectMany(data => data.GetNodes()).ToArray()))
           .AddIndependentVariable(GetGrindData("mining",
                ProfessionsData.Where(data => data.Profession is "Mining")
                               .SelectMany(data => data.GetNodes()).ToArray()))
           .AddIndependentVariable(new MappedVariable<string, string>("enemy_data",
                EnemyListData.ToDictionary(data => $"\"{data.Name}\"",
                    data => $"[{data.Level}, [{string.Join(", ", data.Areas.Select(s => $"\"{s}\""))}]]")))
           .AddIndependentVariable(new MappedVariable<string, string>("portal_counts",
                LocationLevelData.ToDictionary(data => $"\"{data.Area}\"", data => $"{data.ProgressivePortalCount}")));
    ...
    public override void Items(WorldFactory _, ItemFactory item_fact)
    {
        ...
        item_fact
           .ForEachOf(
                ...
                pair => item_fact.AddItemCountVariable($"item_counts_{pair.Key}".ToLower(), pair.Value, pair.Key,
                    addToList: false))
           .ForEachOf(ItemData.GroupBy(data => data.Classification),
                data => item_fact.AddItemListVariable($"{data.Key}_items".ToLower(), data.Key,
                    list: data.Select(d => d.Name).ToArray()))
           .AddItemCountVariable("filler_weights",
                ItemData.Where(data => data.FillerWeight > 0)
                        .ToDictionary(data => data.Name, data => data.FillerWeight),
                Deprioritized, addToList: false)
           .AddItemListVariable("portals", Progression,
           ...
           .AddCreateItems(factory =>
                factory
                   ...
                   .AddCode(new IfFactory("options.random_portals").AddCode(CreateItemsFromList("portals")).SetElse(
                        new CodeBlockFactory().AddCode(CreateItemsFromCountGenCode(
                   ...
                   .AddCode(CreateItemsFillRemainingWithItem("random.choices(filler_items, filler_weightings)[0]",
                        false)));
    ...
    {
        rule_fact
           .AddLogicFunction("area", "has_area",
                $$"""
                  ...
                  """, "area")
           .AddLogicFunction("any_area", "has_any_area", "return any(has_area(state, player, area) for area in areas)",
                "areas")
           .AddLogicFunction("all_areas", "has_all_areas",
           ...
           .AddLogicFunction("grind", "can_grind",
                """
           ...
           .AddCompoundLogicFunction("enemy", "can_beat_enemy",
                "level[enemy_data[enemy_name][0]] and any_area[enemy_data[enemy_name][1]]",
                "enemy_name")
           .AddLogicRules(
           ...
           .AddLogicRules(MerchantData.SelectMany(data => Enumerable.Range(1, 5).Select(i =>
                                           ($"Buy Item #{i} from {data.Name}", $"area[\"{data.Area}\"]")))
                                      .ToDictionary(t => t.Item1, t => t.Item2)).AddLogicRules(Enumerable
               .Range(1, 10)
               .Select(i => ($"Fishing Lv. {i}", $"fish[{i}]"))
               .ToDictionary(t => t.Item1, t => t.Item2))
           .AddLogicRules(Enumerable.Range(1, 10).Select(i => ($"Mining Lv. {i}", $"mine[{i}]"))
                                    .ToDictionary(t => t.Item1, t => t.Item2))
           .AddLogicRules(AchievementData.Where(data => data.Enabled)
                                         .ToDictionary(data => data.Name, data => data.GenRule()));
    }
        ...
        region_fact.AddRegions(LocationLevelData.Select(data => data.Area).ToArray())
                   ...
                   .AddLocations("",
                        AchievementData
                           ...
                           .ToArray())
                   .AddEventLocationsFromList("quests", "f\"Quest Completion: {location[0]}\"",
                        "f\"Complete: {location[0]}\"").ForEachOf(AchievementData
                                                                 .Where(data => data is
                                                                  {
                                                                      Enabled: true, Class: not ClassType.Any
                                                                  })
                                                                 .GroupBy(data => data.Class),
                        (b, g) => b.AddLocations($"options.is_class(\"{g.Key}\")".ToLower(),
    ...
    public override void Init(WorldFactory factory, WorldInitFactory init_fact)
    {
        ...
        init_fact.UseInitFunction()
                 ...
                 .AddCreateItems()
                 .UseSetRules(method =>
                      method.AddCode(new MatchFactory("options.goal")
                                    .AddCase("0", CreateGoalCondition("enemy['Slime Diva']", rule_fact))
                                    .AddCase("1", CreateGoalCondition("enemy['Lord Zuulneruda']", rule_fact))
                                    .AddCase("2", CreateGoalCondition("enemy['Colossus']", rule_fact))
                                    .AddCase("3", CreateGoalCondition("enemy['Galius']", rule_fact))
                                    .AddCase("4", CreateGoalCondition("enemy['Lord Kaluuz']", rule_fact))
                                    .AddCase("5", CreateGoalCondition("enemy['Valdur']", rule_fact))
                                    .AddCase("6",
                                         CreateGoalCondition(string.Join(" and ",
                                             EnemyListData.Where(data => data.IsBoss)
                                                          .Select(data => $"enemy[\"{data.Name}\"]")), rule_fact))
                                    .AddCase("7", CreateGoalCondition(string.Join(" and ", QuestData
                                        .Where(data => data.Enabled
                                                       && QuestData.All(qData => qData.PrevQuest != data.Quest))
                                        .Select(data => $"quest[\"{data.Quest}\"]")), rule_fact))
                                    .AddCase("8", CreateGoalCondition("level[32]", rule_fact))))
                 .UseFillSlotData()
    ...
    {
        progressiveItemMapMd.AddRange("<details>",
            $"<summary><h1 style=\"display: inline\">{className} Progressive Items</h1></summary>");
        item_fact
           .AddItemCountVariable($"{className.ToLower()}_progressives",
                EquipmentItems
                   .Select(kv1 =>
                    {
    ...
    )
    {
        return new GraphBuilder(GameName)
              ...
              .ForEachOf(QuestData.Where(data => data.Enabled),
                   (b, data) => b.AddEventLocation(data.AreaAccepted, getRule, $"Quest Completion: {data.Quest}",
                       data.Quest,
                       $"Complete: {data.Quest}")).GenString();
    }
}